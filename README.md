...
1. [**기본 SELECT 구문**](#1-기본-select-구문)
2. [**WHERE 절과 조건 연산자**](#2-where-절과-조건-연산자)
3. [**SQL 함수**](#3-sql-함수)
4. [**그룹 함수와 GROUP BY**](#4-그룹-함수와-group-by)
5. [**집합 연산자**](#5-집합-연산자)
6. [**JOIN**](#6-join)
7. [**서브쿼리 (Subquery)**](#7-서브쿼리-subquery)
8. [**DML (데이터 조작어)**](#8-dml-데이터-조작어)
9. [**트랜잭션 (Transaction)**](#9-트랜잭션-transaction)
10. [**DDL (데이터 정의어)**](#10-ddl-데이터-정의어)
11. [**제약 조건 (Constraints)**](#11-제약-조건-constraints)
12. [**뷰 (View)**](#12-뷰-view)
13. [**시퀀스 (Sequence) 와 인덱스 (Index)**](#13-시퀀스-sequence-와-인덱스-index)
14. [**사용자 계정 및 권한**](#14-사용자-계정-및-권한)

-----

## \# 1. 기본 SELECT 구문

  - 테이블의 특정 컬럼을 조회하거나 모든 컬럼(`*`)을 조회합니다.
  - 컬럼에 별칭(Alias)을 지정하여 출력할 수 있습니다. `AS` 키워드는 생략 가능합니다.
  - 산술 연산자(`+`, `-`, `*`, `/`)를 사용하여 컬럼 값을 계산할 수 있습니다.
  - `||` 연산자를 사용하여 문자열을 연결할 수 있습니다.
  - `DISTINCT` 키워드로 중복된 행을 제거합니다.
  - `ROWID`는 데이터의 물리적 주소값, `ROWNUM`은 조회 순서에 따른 일련번호입니다.
...
-- 특정 컬럼 조회 및 별칭 사용
SELECT FIRST_NAME AS 이름, SALARY * 12 "1년 연봉" FROM EMPLOYEES;
